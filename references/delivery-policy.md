...
- During task design, classify every missing external credential or tool authorization as `mockable`, `equivalent-fallback`, or `non-substitutable`. For `mockable`, use a contract-faithful mock/fixture and continue local delivery; label the test evidence as simulated, list the live calls not executed, and limit `verified-complete` to the mock-scoped behavior. For `equivalent-fallback`, use and record the approved runner or evidence source. For `non-substitutable`, notify the user as soon as identified with its purpose, secure configuration location, affected TEST/task, and latest-needed stage; never ask for the secret in chat, and block only the affected execution or gate.
