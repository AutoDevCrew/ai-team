...
- If TestSprite is selected, check MCP and API-key availability at readiness. When unavailable, ask once whether to configure it before its first planned execution. On explicit approval, guide host-level MCP setup with `npx @testsprite/testsprite-mcp@latest`, store `API_KEY` only in the host MCP environment, reload the host, and verify that its tools are discoverable without exposing the key. On refusal or setup failure, record Playwright as the equivalent fallback and do not ask again unless the user reopens the choice; if TestSprite itself is an explicit acceptance requirement and no equivalent fallback is authorized, treat its credential as non-substitutable.
