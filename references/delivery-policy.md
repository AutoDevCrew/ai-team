...
- Classify the code baseline as `existing-code` when the repository already contains business or test source, otherwise `greenfield`. Before product analysis, engineering-baseline work, or task design in an existing-code repository, the technical lead must initialize repository discovery with Repomix: use an installed `repomix` first, otherwise run `npx --yes repomix@latest` (transient package execution; no global install). Resolve the version with `<runner> --version`, then run `<runner> --compress --style xml --output <temporary-path> --ignore ".ai-team/**,node_modules/**,vendor/**,dist/**,build/**,coverage/**,.venv/**,**/.env*,**/*secret*,**/*credential*" <repository-root>`. Record the resolved version, runner, exact command, packed scope, exclusions, and file/token metrics in the source register. Keep the raw pack temporary and local. If neither runner is available or Repomix fails, stop repository initialization and request only the minimum Node/npx or Repomix setup; targeted search is not a substitute for this initialization gate. Greenfield intake records a reasoned Repomix N/A.
...
- Before Standard/High-risk task promotion, freeze one project acceptance specification with an explicit REQ catalog and one project traceability matrix. The catalog lists every evaluated REQ, including out-of-scope items; every catalogued requirement appears exactly once as covered, out of scope with its source/decision rationale, or awaiting decision. Only covered rows map AC, TASK, and TEST IDs. A standalone Fast non-behavior task may use its card-local source/traceability instead when both project specification files are intentionally absent; if either project file exists, validate both normally.
...
- An unavailable optional Figma, TestSprite, browser route, or convenience tool is not by itself a blocker. Continue with targeted source/code evidence or provider-neutral tests and record the limitation. Repomix is not optional for existing-code initialization. Treat any other unavailable tool as a hard evidence gap only when that source or capability is explicitly required for acceptance or is the sole authority for a material rule.
- During source intake and engineering-baseline work, inventory applicable external integrations and tool authorizations before affected task design, using the exact Schema/template fields. Declare `external-integration` on every affected task. Record key names, contract source, treatment, safe local target, action/readiness status, mock/fallback and live TEST IDs, affected task and latest-needed gate, and redacted probe evidence. Consolidate user action into one notice grouped as needed now, safely deferrable, or optional; do not create a decision card unless a material product, cost, permission, or provider choice remains.
- Prefer the project's existing secret-loading convention. When an integration is evidenced and the local target is absent, Codex may create a valid local-only configuration scaffold during analysis with required key names, blank secret values, safe comments, and known non-secret defaults. Create it only after confirming the application reads that exact location and the target is excluded from version control or other publication; add only the narrow ignore rule when project authority permits. Never overwrite or rewrite an existing secret-bearing file, copy a secret from another location, put credentials under `.ai-team/`, use non-empty fake secret placeholders, or echo values. If the target exists, report only missing key names and let the user edit it locally.
- Mockability changes when work blocks, never whether a missing credential is disclosed: every missing credential stays in the baseline and consolidated user-action notice. For `mockable`, continue only from a known contract through a provider-neutral adapter and contract-faithful mock/fixture; freeze both mock and later live authentication/contract TEST IDs, label evidence simulated, list unexecuted live calls, and limit `verified-complete` to mock-scoped behavior until the declared live gate. Missing contract evidence is not safely mockable. For `equivalent-fallback`, use and record the approved runner or evidence source. For `non-substitutable`, notify as soon as identified and block only the affected execution or declared gate. Never ask the user to paste a secret into chat.
...
- For Web UI scope, the independent verifier defines and freezes provider-neutral TEST IDs, expected results, accounts/data/reset, and the intended UI runner during task design. Prefer an existing project runner. Check availability at readiness, but do not execute unfinished-feature acceptance tests or record functional PASS evidence.
- If the user explicitly instructs Codex to use TestSprite, treat that instruction as approval for its minimum reversible host-level MCP package installation and configuration at readiness; do not ask again for installation approval. If the team selects TestSprite without that instruction, ask once before setup. Use `npx @testsprite/testsprite-mcp@latest`, keep `API_KEY` only in the host MCP environment, reload the host, and verify tool discovery without exposing the key. This authority excludes obtaining or moving secrets, paid-plan purchase, administrator actions, runtime replacement, production access, and deployment; request only the missing authority through a secure configuration path. If TestSprite is preferred, setup refusal or failure falls back to Playwright; if it is an explicit acceptance requirement and no fallback is authorized, treat it as non-substitutable and block only the affected UI execution or gate.
- Execute the frozen UI cases only after the affected implementation exists: the serial implementer runs the approved subset as self-check, then the independent verifier reruns the same TEST IDs on a fresh candidate and performs required regression. TestSprite is optional authorized local Web UI automation and never replaces unit, integration, or contract tests.
- If Playwright is selected and absent, install it project-locally with the detected package manager only when the current implementation or verification authority permits test-tool changes; reuse the lockfile and never install it globally. `analysis-only` work may plan the cases and runner but may not install dependencies. Never claim unavailable-tool evidence as PASS, expose credentials, target production, or deploy without separate authority.
