...
- When intake includes a local directory, archive, or multiple supplied artifacts, mark delivery-package coverage applicable before product analysis. Expand supplied archives, then create `.ai-team/evidence/intake-package.json` with `python3 .ai-team/scripts/intake_package_inventory.py snapshot <package-root> --output .ai-team/evidence/intake-package.json`. Classify bounded paths with `mark`, for example `mark <manifest> --status reviewed --path <file> --evidence <EVID-anchor>` or `mark <manifest> --status excluded --prefix <directory> --reason <rationale>`; reviewed items require evidence, while excluded or gap items require a concrete reason. Do not silently sample. A single registered URL or verbatim request with no multi-file package records a reasoned coverage N/A.
- Do not claim intake complete or start product analysis until `python3 .ai-team/scripts/intake_package_inventory.py verify .ai-team/evidence/intake-package.json` passes, its computed counts close with `gap=0`, and an independent verifier records an intake PASS. Full project audits repeat the source rescan; routine task gates validate the frozen manifest without rescanning the package. This proves file-level coverage and freshness, not semantic comprehension. An inaccessible or unsupported item remains an explicit gap; it is not evidence of absence.
...
- Run the scoped gate at routine task boundaries and the full audit at its declared mandatory triggers. Repair its source-located `fix-consistency` action on FAIL, or dispatch/execute its eligible local action on PASS instead of returning a next-step report.
