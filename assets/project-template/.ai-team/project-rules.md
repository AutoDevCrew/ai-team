...
- Before recording `task-design-ready`, before `verified-complete`, and before crossing a High-risk channel, run `scripts/validate_task_handoff.py <task-card> --strict`. Code/protocol/generated-output/dependency/runtime/High-risk tasks use `Fingerprint policy: required`; pure Fast-path documentation/style/metadata tasks may use a reasoned `N/A`. Strict validation verifies the required SHA-256 ledger automatically and does not execute project commands or prove test evidence truth.
...
- Choose a task lane before design: use `fast` first for the positive whitelist of pure documentation/comments/copy/style-token changes or local test additions with no business behavior change, then confirm no contract, security, data, dependency, runtime-chain, UX, or baseline trigger; use `standard` for ordinary M/L or shared-surface work; use `high-risk` for XL or any security, permission, sensitive-data, protocol, migration, transaction, worker/async, external-side-effect, or production-capability boundary. Record the lane. Fast path may merge design/readiness review and omit the runtime-chain matrix only when its exclusion criteria remain true; it still requires independent verification and scope checks.
