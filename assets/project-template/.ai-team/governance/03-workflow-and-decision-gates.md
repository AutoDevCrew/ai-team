...
The independent verifier freezes a **Test Execution Manifest** before implementation. It divides a fast-gate for critical contracts/security, owner and affected/regression checks used while iterating, one approved full suite after the final revision, and independent risk/mutation checks. For each candidate revision, the code/security reviewer completes the scoped fast-gate first; do not start the expensive independent full suite while a deterministic P0/P1 or invalid evidence remains. The implementation engineer runs focused checks while iterating and one approved full suite after its final revision; only after the scoped review passes does the verifier run one fresh full suite. The code/security reviewer does not duplicate that full suite. Capture a deterministic P0/P1, record partial-execution evidence (manifest revision, executed groups/results, unexecuted groups, and stop reason), stop unrelated broad execution, return to focused rework, then obtain final valid evidence. The reviewer exhausts all checks in its declared scope unless a P0, environment failure, or evidence invalidation prevents meaningful continuation.
