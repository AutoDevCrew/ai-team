...
Record the same `DEC-<NNN>` ID in `decisions.md` when it first blocks work. Update its `Status` as evidence or the human response arrives; only `Status: confirmed` clears the blocker.
...
- Design/readiness verdict and conditions: task-design-ready / implementation-ready / conditional-pass with exact activation evidence / blocked with reason; retained L/XL also records `split-decision: retained — <concrete stop-splitting rationale>`
...
- Stage binding: <stage Updated at> / <stage SHA-256> when pre-task validity depends on stage; N/A — task-bound gate or concrete stage-independent rationale
